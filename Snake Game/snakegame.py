...
def drawTrap(canvas, data):
    ...
    if (data.trap_center != None):
        ...
        if (data.direction____ == 0):
            canvas.create_rectangle(x0 - 3 * data.cols, y0, x1 + 3 * data.cols, y1, fill="red")

        elif (data.direction____ == 1):
            canvas.create_rectangle(x0, y0 - 3 * data.cols, x1, y1 + 3 * data.cols, fill="red")
...
def start_mode_RedrawAll(canvas, data):
    canvas.create_text(data.width / 2, data.height / 2 - 100, text='Welcome to Game', font='Arial 30')
...
def game_mode_RedrawAll(canvas, data):
    ...
    canvas.create_text(50, 315, text=int(data.time), fill='White', font='Arial 20')
...
def end_mode_RedrawAll(canvas, data):
    if (data.lives == 0):
        canvas.create_text(data.width / 2, data.height / 2 - 100, text='You Lose, GAME IS OVER', font='Arial 20')
    elif (data.lives > 0):
        canvas.create_text(data.width / 2, data.height / 2 - 100, text='You lose', font='Arial 30')
        canvas.create_text(data.width / 2, data.height / 2 + 30, text='You score is %d' % (data.score), font='Arial 20')
        canvas.create_text(data.width / 2, data.height / 2 + 50, text='You have %d lives left' % (data.lives),
                           font='Arial 20')
